...
class C3D_FROZEN(Abstract_Model_Class):
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(C3D_FROZEN, self).__init__(**kwargs)
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, dropout_rate = 0.5, return_layer=['logits'], weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating C3D Frozen network layers')

        # END IF

        with tf.name_scope(scope, 'c3d_frozen', [inputs]):
            layers = {}

            layers['conv1'] = conv3d_layer(input_tensor=inputs,
                    filter_dims=[3, 3, 3, 64],
                    name='c1',
                    weight_decay = weight_decay, non_linear_fn=tf.nn.relu, trainable=False)
            ...
            layers['dense1'] = fully_connected_layer(input_tensor=layers['reshape'],
                                                     ...
                                                     name='d1', weight_decay=weight_decay)#, trainable=False)
    ...
    def loss(self, logits, labels, loss_type):
        ...
        if loss_type=='full_loss':
            loss_value = tf.losses.sparse_softmax_cross_entropy(labels=labels,
                                                                    logits=logits)

        else:
            loss_value = tf.losses.sigmoid_cross_entropy(multi_class_labels=labels,
                    logits=logits[:,:,0])
