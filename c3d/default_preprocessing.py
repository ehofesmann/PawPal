...
import numpy as np
from c3d.preprocessing_utils import *
...
def preprocess(input_data_tensor, frames, height, width, channel, input_dims, output_dims, seq_length, size, label, istraining, input_alpha=1.0):
    ...
    _mean_image = tf.map_fn(lambda img: resize(img, size[0], size[1]), tf.convert_to_tensor(_mean_image[...,::-1].tolist()))

    input_data_tensor = tf.map_fn(lambda img: resize(img, size[0], size[1]), input_data_tensor)
